...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const blendModes = ['color', 'color-burn', 'color-dodge', 'darken', 'difference', 'exclusion', 'hard-light', 'hue', 'lighten', 'luminosity', 'multiply', 'normal', 'saturation', 'screen', 'soft-light'];
  let curModeIdx = 0;
  const bodyEl = document.querySelector('body');
  function changeBGStyle() {
    bodyEl.style.backgroundBlendMode = blendModes[curModeIdx];
    curModeIdx += 1;
    if (curModeIdx >= blendModes.length)
      curModeIdx = 0;
  }
